...
{
   RegisterDevice(g_DeviceName, MM::ShutterDevice, "Official Cobolt Laser Controller");
}
                     ...
                     analogImpedanceState_(g_Default_Str_Unknown)
{
   ...
   /* Define error texts, for CoboltOfficial Error codes (see headerfile) */
   ...
   // Port
   CPropertyAction* pAct = new CPropertyAction (this, &CoboltOfficial::OnPort);
   CreateProperty(MM::g_Keyword_Port, g_Default_Str_Unknown, MM::String, false, pAct, true);
...
int CoboltOfficial::Initialize()
{
   ...
   else if (strcmp(port_.c_str(), g_Default_Str_Unknown) != 0)
   {
      ...
      pAct = new CPropertyAction (this, &CoboltOfficial::OnVersion);
      ...
      pAct = new CPropertyAction (this, &CoboltOfficial::OnLaserPowerSetting);
      ...
      pAct = new CPropertyAction (this, &CoboltOfficial::OnLaserCurrentSetting);
      ...
      pAct = new CPropertyAction (this, &CoboltOfficial::OnOperatingMode);
      ...
      pAct = new CPropertyAction (this, &CoboltOfficial::OnLaserOutput);
      ...
      if (DEVICE_OK == nRet)
      {
         pAct = new CPropertyAction (this, &CoboltOfficial::OnModulationLaserPower);
...
int CoboltOfficial::OnVersion(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
      /* Read Only, i.e. always set property according to member value. */
...
int CoboltOfficial::OnLaserPowerSetting(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   double value;
   int reply = DEVICE_OK;
   ...
   {
      pProp->Get(value);
      reply = SetLaserPowerSetting(value);
      if (reply != DEVICE_OK)
      {
...
int CoboltOfficial::OnLaserCurrentSetting(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   double value;
   int reply = DEVICE_OK;
...
int CoboltOfficial::OnLaserOnOff(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   if (eAct == MM::BeforeGet)
   {
      laserStatus_ = GetLaserStatus();
...
int CoboltOfficial::OnOperatingMode(MM::PropertyBase* pProp, MM::ActionType eAct)

{
   ...
   else if (eAct == MM::AfterSet)
   {
      ...
      else
      {
         ...
         LogMessage("CoboltOfficial::OnOperatingMode: Invalid LaserOperating Mode. Got: " + answer, true);
         return DEVICE_INVALID_INPUT_PARAM;
      }
...
int CoboltOfficial::OnLaserOutput(MM::PropertyBase* pProp, MM::ActionType eAct)
{
...
int CoboltOfficial::OnModulationLaserPower(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   double value;
   int reply = DEVICE_OK;
   ...
   if (eAct == MM::BeforeGet)
   {
      pProp->Set(modulationPowerSetting_);

   }
   else if (eAct == MM::AfterSet)
   {
      pProp->Get(value);
      reply = SetModulationPowerSetting(value);
      if (reply != DEVICE_OK)
      {
         /* Failed to change modulation power setting. Fetch current power and update property */
         pProp->Set(modulationPowerSetting_);
      }
   }
 ...
 */
int CoboltOfficial::SendSerialCmd(std::string command, std::string& answer)
{
   int reply;

   reply = SendSerialCommand(port_.c_str(), command.c_str(), g_SendTerm);
   if (reply != DEVICE_OK)
   {
   ...
   else
   {
      ...
      /* TODO: If unsupported command. Do Cobolt-lasers return below? */
...
double CoboltOfficial::GetLaserPowerSetting()
{
   ...
   if (reply != DEVICE_OK)
   {
      ...
      power_mW = 0.0; /* FIX: What to do when unknown power??? */
   }
   else
   {
...
int CoboltOfficial::SetLaserPowerSetting (double power_mW)
{
   std::string answer;
   int reply;
   std::string cmd;
...
std::string CoboltOfficial::GetLaserStatus()
{
   std::string answer;
   int reply;
...
{
   std::string answer;
   int reply = SendSerialCmd("i?", answer); /* Read drive current (mA) */
